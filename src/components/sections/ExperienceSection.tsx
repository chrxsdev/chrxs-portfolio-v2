    ...
    <section id='experience' className='py-32 px-3 sm:px-6 relative z-10'>
      <div className='max-w-5xl mx-auto'>
        ...
        <div ref={containerRef} className='relative'>
          ...
          {experience && (
            <div className='absolute left-[31px] md:left-1/2 md:-translate-x-1/2 top-0 bottom-0 w-[4px] bg-border' />
          )}

          {/* Timeline Animated Line (fills as you scroll) */}
          {experience && (
            <motion.div
              className='absolute left-[31px] md:left-1/2 md:-translate-x-1/2 top-0 w-[3px] bg-gradient-to-b from-minimal-green to-minimal-lemon origin-top'
          ...
          <div className='space-y-16 md:space-y-24'>
            {experience ? (
              experience.map((exp, index) => (
                <RevealOnScroll key={exp.id} delay={index * 0.1} direction='up' distance={50}>
                  <div
                    className={`relative flex flex-col md:flex-row items-start gap-6 md:gap-12 md:self-center ${
                      index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'
                    }`}
                  >
                    {/* Timeline Node */}
                    <div className='absolute left-[23px] md:left-1/2 md:-translate-x-1/2 md:top-22 top-8 z-20'>
                      <motion.div
                        className={`w-4 h-4 rounded-full border-3 border-background ${
                          exp.current_job
                    ...
                    <div
                      className={`flex-shrink-0 ml-16 md:ml-0 md:w-[calc(50%-3rem)] md:self-center ${
                        index % 2 === 0 ? 'md:flex md:justify-end' : 'md:flex md:justify-start'
                      }`}
                    >
                      <div className='w-20 h-20 rounded-full bg-gradient-to-br from-minimal-green to-minimal-lemon p-0.5 shadow-lg'>
                    ...
                    {/* Experience Card */}
                    <div className={`ml-16 md:mx-8 md:w-[calc(50%-3rem)]`}>
                      <motion.div
                        className={`bg-card/40 backdrop-blur-sm rounded-2xl p-6 border transition-all duration-300 group cursor-pointer ${
                          exp.current_job
                        ...
                        <div className='flex flex-col space-y-2'>
                          ...
                          <div className='flex flex-col gap-2 mt-2'>
                            <div className='flex items-center gap-2 text-sm text-muted-foreground'>
                              ...
                              <span className='px-1 py-0.5 rounded-md bg-muted text-xs'>
